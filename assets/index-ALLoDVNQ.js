...
`,se=e=>{let t=Number(e);return Number.isNaN(t)?e:`${t.toLocaleString()}원`},ce=(e,t)=>{if(typeof e!=`number`)return``;let n=Math.round(e),r=`
  ...
  `,ue=({product:e,loading:t=!1,error:n=null}={})=>{if(t)return oe;if(n)return h(n);if(!e)return h(`상품 정보를 찾을 수 없습니다.`);let{title:r,description:i,image:a,lprice:o,mallName:s,stock:c,rating:l,reviewCount:u,brand:d,maker:f,relatedProducts:p=[]}=e,ee=se(o),te=ce(l,u),m=[d,f,s].filter(Boolean).join(` • `);return`
      ...
      <section class="bg-white rounded-lg shadow-sm">
        <div class="p-4">
          ...
          <div class="mt-4 space-y-4">
            <header>
              <p class="text-sm text-gray-500 mb-1">${m}</p>
              <h1 class="text-xl font-bold text-gray-900 mb-3">${r}</h1>
              ${te}
            </header>
            <div>
              <span class="text-2xl font-bold text-blue-600">${ee}</span>
